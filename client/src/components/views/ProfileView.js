...
const ProfileView = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const biography = e.biography;
    const username = e.username;
    const email = e.email;
    const phonenumber = e.phonenumber;
    const address = e.address;

    await updateUser(user, { biography, username, email,phonenumber,address });

    setProfile({ ...profile, user: { ...profile.user, biography, username, email, phonenumber, address } });
    setEditing(false);
};
  ...
  const validate = (content) => {
    ...
    if (content.length > 250) {
      error = "Tiêu đề không thể dài hơn 250 từ";
    }
